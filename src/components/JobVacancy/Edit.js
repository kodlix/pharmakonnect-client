...
import { editJobVacancy, loadJobVacancyByCompany, formatter } from '../../store/modules/jobVacancy';
...
const Edit = (props) => {
    const jobVacancyFormKeys = ["contactType", "jobDescription", "jobLocation", "jobTitle", "maxSalary", "minSalary", "minimumQualification", "nameOfCorporation", "workExperienceInYears", "yearOfIncorporation"]
    const { register, handleSubmit, errors, setValue, control, watch, trigger, getValues } = useForm({
        mode: "onChange",
        reValidateMode: 'onChange'
    });
    const dispatch = useDispatch();
    const jobId = props.match.params.id;
    const jobVacancy = useSelector(state => state.jobVacancy.jobVacancyByCompany);
    ...
    const onSubmit = (job) => {
        ...
        dispatch(editJobVacancy(jobId, job));

    }
    useEffect(() => {
        if (Object.keys(jobVacancy).length > 0) {
            dispatch(allowOwnerOnly(jobVacancy.accountId, '/jobvacancy'));
            jobVacancyFormKeys.map(key => { setValue(key, jobVacancy[key]) })
            setJobDescriptionValue(jobVacancy.jobDescription);
            setValue("startDate", new Date(jobVacancy.startDate));
            setValue("endDate", new Date(jobVacancy.endDate));
            setValue("yearOfIncorporation", new Date(jobVacancy.yearOfIncorporation));
        }
    }, [jobVacancy]);
    ...
    return (
        <React.Fragment>
            <form onSubmit={handleSubmit(onSubmit)} className="col-lg-9 col-md-8 col-12 mb-5">
                <div className="">
                    <PageHeader pageTitle={'Edit Job Vacancy'} heading={jobVacancy?.jobTitle}>
                    ...
                    <div className="shadow p-3 mt-1 bg-white rounded">
                        ...
                        <div className="row">
                            ...
                            <div className="mb-3 col-md-6">
                                ...
                                {jobVacancy.yearOfIncorporation && <Controller
                                    control={control}
                                    ...
                                    render={(props) => (
                                        <DatePicker
                                            ...
                                            onChange={(e) => setValue("yearOfIncorporation", e)}
                                            selected={getValues("yearOfIncorporation")}
                                            showYearPicker
                                            dateFormat="yyyy"
                                        />
                                    )}
                        ...
                        <div className="row">
                            <div className="mb-3 col-md-6">
                                <label htmlFor="companyUrl" className="form-label">Company Website<span className="text-danger font-weight-bold">{errors.companyUrl && " * Company URL is required"}</span></label>
                        ...
                        <div className="row">
                            ...
                            <div className="mb-3 col-md-6">
                                <label htmlFor="jobUrl" className="form-label">Job URL<span className="text-danger font-weight-bold">{errors.jobUrl && " * Job URL is required"}</span></label>
                                <input type="text" className="form-control" name="jobUrl" id="jobUrl" placeholder="Job Website" ref={register({
                                    pattern: {
                                        value: /((([A-Za-z]{3,9}:(?:\/\/)?)(?:[-;:&=\+\$,\w]+@)?[A-Za-z0-9.-]+|(?:www.|[-;:&=\+\$,\w]+@)[A-Za-z0-9.-]+)((?:\/[\+~%\/.\w-]*)?\??(?:[-\+=&;%@.\w]*)#?(?:[\w]*))?)/,
                                        message: "Job URL format is invalid"
                                    }
                                })} />
                            </div>
                        </div>
                            ...
                            <div className="row">
                                <div className="mb-3 col-md-6">
                                    ...
                                    <Controller
                                        ...
                                        DefaultValue={jobVacancy.startDate}
                                        name="startDate"
                                        control={control}
                                        // valueName="selected"
                                        rules={{
                                ...
                                <div className="mb-3 col-md-6">
                                    ...
                                    <Controller
                                        ...
                                        control={control}
                                        // valueName="selected"
                                        DefaultValue={jobVacancy.endDate || new Date()}
                                        rules={{
                    ...
                    <div className="shadow p-5  mt-5 mb-3 bg-white rounded">
                        <h5 className="project-text-color">Job Description * {errors.jobDescription && <span className="text-danger font-weight-bold">{errors.jobDescription.message}</span>}{errors.jobDescription && errors.jobDescription.type === "validate" && <span className="text-danger font-weight-bold"> Job Description is required</span>}</h5>
                    ...
                    <div className="text-right"><button type="submit" className="btn btn-success btn-reg fw-bold shadow p-3 mt-3 mb-5 rounded">Submit</button>
                    </div>
                </div>
            </form>
        </React.Fragment>
